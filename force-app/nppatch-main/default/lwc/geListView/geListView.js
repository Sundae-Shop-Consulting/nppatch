...
import { dispatch, handleError, generateId } from "c/utilTemplateBuilder";
import { deepClone, format, getNamespace, hasNestedProperty, isNotEmpty, showToast } from "c/utilCommon";
...
const _SELF = "_self";
const SAVE = "save";
const EXCLUDED_COLUMN_HEADERS = ["CloneSourceId", "SystemModstamp"];
...
export default class geListView extends LightningElement {
    ...
    get iconName() {
        ...
        return this.objectInfo && this.objectInfo ? `${SLDS_ICON_CATEGORY_STANDARD}:${this.objectInfo.apiName}` : "";
    }
    ...
    handleImperativeRefresh = async () => {
        ...
        await this.getRecords(this.columns).catch((error) => {
            handleError(error);
        });
    ...
    wiredObjectInfo(response) {
        ...
        if (response.error) {
            ...
            this.informGiftEntryHomeApp(
                "listViewPermissionsChange",
                this.CUSTOM_LABELS.geErrorObjectCRUDHeader,
                GeLabelService.format(this.CUSTOM_LABELS.geErrorObjectCRUDBody, [this.objectApiName])
            );
        }
    }
    ...
    init = async () => {
        ...
        const columnHeaderData = await this.getColumnHeaderData(this.listName).catch((error) => {
            handleError(error);
        });
        ...
        if (isNotEmpty(flsErrors)) {
            ...
            this.informGiftEntryHomeApp(
                ...
                GeLabelService.format(this.CUSTOM_LABELS.geErrorFLSBody, [flsErrors])
            );
            return;
        }
    ...
    getColumnHeaderData = async (listViewDeveloperName) => {
        return retrieveCustomColumnHeaders({ listName: listViewDeveloperName }).catch((error) => {
            handleError(error);
        });
    };
    ...
    buildBaseColumnEntry(fieldDescribe) {
        ...
        return {
            fieldApiName: isNotEmpty(referenceField) ? referenceField : fieldDescribe.apiName,
    ...
    buildTypesAttribute(columnEntry, fieldApiName) {
        ...
        columnEntry.typeAttributes = this.getColumnTypeAttributes(columnEntry.type, fieldApiName);
        return columnEntry;
    }
    ...
    getUserTimeZone() {
        const hasUserTimezoneData =
            this.wiredUserRecord.data &&
            this.wiredUserRecord.data.fields &&
            this.wiredUserRecord.data.fields.TimeZoneSidKey.value;
        return hasUserTimezoneData ? this.wiredUserRecord.data.fields.TimeZoneSidKey.value : undefined;
    }
    ...
    setDatatableRecordsForImperativeCall(dataRecords) {
        ...
        records.forEach((record) => {
            Object.keys(record).forEach((key) => {
                ...
                if (isNotEmpty(fieldDescribe) && (fieldDescribe.nameField || fieldDescribe.reference)) {
                    const _objectApiName = fieldDescribe.nameField
    ...
    getRecordUrl(objectApiName) {
        ...
        if (objectApiName === FORM_TEMPLATE_INFO.objectApiName) {
            const giftEntryTabName = TemplateBuilderService.alignSchemaNSWithEnvironment(
                "GE_Gift_Entry",
                this.namespace
            );

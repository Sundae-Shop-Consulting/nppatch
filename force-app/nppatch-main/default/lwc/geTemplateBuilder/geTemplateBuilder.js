...
export default class geTemplateBuilder extends NavigationMixin(LightningElement) {
    ...
    wiredBatchDataImportObject({ error, data }) {
        ...
        if (data) {
            ...
            this._dataImportBatchPermissionErrors = this.checkSingleFieldPermission(
                this.diBatchInfo,
                DATA_IMPORT_BATCH_TABLE_COLUMNS_FIELD.fieldApiName
            );
    ...
    get disableBatchTableColumnsSubtab() {
        if (!this._dataImportBatchPermissionErrors) {
            return false;
        }

        this._batchTableLabelAccessErrorLabel = this._dataImportBatchPermissionErrors.noAccessFields.find((field) =>
            field.includes(DATA_IMPORT_BATCH_TABLE_COLUMNS_FIELD.fieldApiName)
        );
    ...
    get batchTableColumnsAccessErrorMessage() {
        if (!this.disableBatchTableColumnsSubtab) {
            return "";
        }

        return GeLabelService.format(this.CUSTOM_LABELS.geErrorFLSBody, [this._batchTableLabelAccessErrorLabel]);
    }
    ...
    buildBatchTableColumnOptions() {
        ...
        this.getFormFieldsFromSections(this.formSections).forEach((formField) => {
            if (formField.elementType === "widget") {
                return;
            }
    ...
    notify(modalData) {
        if (modalData.action === SAVE) {
            ...
            const formSection = formSections.find((fs) => {
                return fs.id === modalData.section.id;
            });
    ...
    getFieldMappingBySourceFieldAndTargetObject() {
        ...
        Object.keys(TemplateBuilderService.fieldMappingByDevName).forEach((key) => {
            ...
            if (fieldMapping.Source_Field_API_Name && fieldMapping.Target_Object_API_Name) {
                const newKey = `${fieldMapping.Source_Field_API_Name}.${fieldMapping.Target_Object_API_Name}`;
    ...
    handleAddFormSection(event) {
        const label =
            event && event.detail && typeof event.detail.label === "string"
                ? event.detail.label
    ...
    announceFieldAddedToSection(formSection, field) {
        ...
        const screenReaderMessage = isNotEmpty(formSection.label)
    ...
    addFieldToAvailableBatchTableColumnOptions(field) {
        ...
        const isAFieldMappingField = fieldMappingDevNames && fieldMappingDevNames[0] && field.elementType === "field";
    ...
    updateOptionFromBatchTableColumn(formField) {
        const fieldMapping = TemplateBuilderService.fieldMappingByDevName[formField.dataImportFieldMappingDevNames[0]];
        if (!fieldMapping) {
            return;
        }
    ...
    handleFormElementUp(event) {
        ...
        let section = this.formSections.find((fs) => {
            return fs.id === sectionId;
        });
        const index = section.elements.findIndex((element) => {
            const name = element.componentName ? element.componentName : element.dataImportFieldMappingDevNames[0];
    ...
    handleDeleteFormElement(event) {
        ...
        const section = this.formSections.find((fs) => {
            return fs.id === sectionId;
        });
        const index = section.elements.findIndex((element) => {
    ...
    validateSelectFieldsTab(tabsWithErrors) {
        ...
        if (selectFieldsComponent) {
            ...
            const missingRequiredFields = findMissingRequiredFieldMappings(TemplateBuilderService, this.formSections);
    ...
    validateBatchHeaderTab(tabsWithErrors) {
        this.missingRequiredBatchFields = findMissingRequiredBatchFields(
        ...
        if (this.missingRequiredBatchFields && this.missingRequiredBatchFields.length > 0) {
            ...
            showToast(
                this.CUSTOM_LABELS.commonWarning,
        ...
        if (tabsWithErrors && !this.disableBatchTableColumnsSubtab) {
            const isMissingBatchTableColumns =
                isEmpty(this.selectedBatchTableColumnOptions) || this.selectedBatchTableColumnOptions.length === 0;
    ...
    handleFormTemplateSave = async () => {
        ...
        if (isTemplateValid) {
            this.isLoading = true;
            try {
                const recordId = await storeFormTemplate(this.preppedFormTemplate());
                if (recordId) {
                    const toastLabel =
                        this.mode === NEW

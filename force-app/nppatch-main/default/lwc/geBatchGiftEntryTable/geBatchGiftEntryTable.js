...
const COLUMNS = [
    ...
    {
        label: geDonorColumnLabel,
        fieldName: DONOR_LINK,
        type: "url",
        typeAttributes: { label: { fieldName: DONOR_NAME } },
    },
    {
        label: geDonationColumnLabel,
        fieldName: MATCHED_DONATION_LINK,
        type: "url",
...
export default class GeBatchGiftEntryTable extends LightningElement {
        ...
        if (this._giftBatchState?.gifts) {
            ...
            this._giftBatchState.gifts.forEach((gift) => {
                ...
                giftViewAsTableRow = Object.assign(
                    ...
                    this.appendUrlColumnProperties.call(giftViewAsTableRow.fields, this._dataImportObjectInfo)
                );
    ...
    get columns() {
        return this.userDefinedBatchTableColumnNames && this.userDefinedBatchTableColumnNames.length > 0
    ...
    get userDefinedColumns() {
        ...
        this.userDefinedBatchTableColumnNames.forEach((columnName) => {
            if (this._columnsBySourceFieldApiName[`${columnName}${URL_SUFFIX}`]) {
                ...
                // intentionally empty - skip undefined columns
            } else {
    ...
    upsertData(dataRow, idProperty) {
        const existingRowIndex = this.data.findIndex((row) => row[idProperty] === dataRow[idProperty]);
    ...
    handleMenuItemSelect(event) {
        if (event.detail.value === "selectcolumns") {
            const selectColumns = new CustomEvent("selectcolumns", {
                detail: {
                    options: this.allColumns.map(({ label, fieldName }) => ({
                        label,
                        value: fieldName,
                    })),
                    values: this.columns.map(({ fieldName }) => fieldName),
                },
            });
     ...
     * @returns {object}
     */
    appendUrlColumnProperties(objectInfo, urlSuffix = URL_SUFFIX, urlLabelSuffix = URL_LABEL_SUFFIX) {
        Object.keys(this)
            .filter((key) => key.endsWith("__r") || this[key]?.attributes)
            .forEach((key) => {
    ...
    populateDonorLink(giftViewAsTableRow) {
        ...
        if (donorType === "Contact1") {
            ...
            giftViewAsTableRow[DONOR_NAME] =
                giftViewAsTableRow[`${ORGANIZATION_IMPORTED.fieldApiName}${URL_LABEL_SUFFIX}`];
        }
    ...
    populateMatchedDonationLink(giftViewAsTableRow) {
        ...
        if (isMatchedToAPaymentRecord) {
            giftViewAsTableRow[MATCHED_DONATION_LINK] =
                giftViewAsTableRow[`${PAYMENT_IMPORTED.fieldApiName}${URL_SUFFIX}`];
            giftViewAsTableRow[MATCHED_DONATION_LABEL] =
                giftViewAsTableRow[`${PAYMENT_IMPORTED.fieldApiName}${URL_LABEL_SUFFIX}`];
        } else if (isMatchedToAnOpportunityRecord) {
            giftViewAsTableRow[MATCHED_DONATION_LINK] =
                giftViewAsTableRow[`${DONATION_IMPORTED.fieldApiName}${URL_SUFFIX}`];
            giftViewAsTableRow[MATCHED_DONATION_LABEL] =
                giftViewAsTableRow[`${DONATION_IMPORTED.fieldApiName}${URL_LABEL_SUFFIX}`];
        } else if (isRecurringGift) {
            giftViewAsTableRow[MATCHED_DONATION_LINK] =
                giftViewAsTableRow[`${RECURRING_DONATION_IMPORTED.fieldApiName}${URL_SUFFIX}`];
            giftViewAsTableRow[MATCHED_DONATION_LABEL] =
                giftViewAsTableRow[`${RECURRING_DONATION_IMPORTED.fieldApiName}${URL_LABEL_SUFFIX}`];
        }
        return giftViewAsTableRow;
    }
    ...
    getColumn(element, fieldMapping) {
        ...
        const columnFieldName = fieldMapping.Source_Field_API_Name.toLowerCase().endsWith("id")
            ? fieldMapping.Source_Field_API_Name.slice(0, -2)
            : fieldMapping.Source_Field_API_Name;

        const column = {
            label: element.customLabel,
            fieldName: isReferenceField ? `${columnFieldName}${URL_SUFFIX}` : fieldMapping.Source_Field_API_Name,
        ...
        if (isReferenceField) {
            ...
            column.typeAttributes = {
                label: {
                    fieldName: `${columnFieldName}${URL_LABEL_SUFFIX}`,
                },
            };
        }
        return column;

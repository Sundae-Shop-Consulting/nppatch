...
export default class geTemplateBuilderBatchHeader extends LightningElement {
    ...
    removeField(index, fieldName) {
        const batchFieldIndex = findIndexByProperty(this.batchFields, PROP_API_NAME, fieldName);
    ...
    handleRequiredFields() {
        const requiredFields = this.batchFields.filter((batchField) => {
        ...
        requiredFields.forEach((field) => {
            if (selectedFieldsExists) {
                const alreadySelected = this.selectedBatchFields.find((bf) => {
    ...
    toggleCheckboxForSelectedBatchFields() {
        if (this.selectedBatchFields && this.selectedBatchFields.length > 0) {
            ...
            for (let i = 0; i < this.selectedBatchFields.length; i++) {
                ...
                const batchFieldIndex = findIndexByProperty(_batchFields, PROP_API_NAME, selectedBatchField.apiName);

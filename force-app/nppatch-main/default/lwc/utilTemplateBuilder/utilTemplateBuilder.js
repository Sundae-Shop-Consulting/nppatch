...
const ADDITIONAL_REQUIRED_BATCH_HEADER_FIELDS = [DI_BATCH_NAME_FIELD_INFO.fieldApiName];
...
const findMissingRequiredFieldMappings = (TemplateBuilderService, formSections) => {
    const requiredFieldMappings = Object.keys(TemplateBuilderService.fieldMappingByDevName).filter((developerName) => {
    ...
    const selectedFieldMappingDevNames = formSections
        .flatMap((section) => section.elements)
        .map((element) => {
            return element.componentName ? element.componentName : element.dataImportFieldMappingDevNames[0];
        });

    const missingRequiredFieldMappings = requiredFieldMappings.filter(
        (developerName) => !selectedFieldMappingDevNames.includes(developerName)
    );
...
const findMissingRequiredBatchFields = (batchFields, selectedBatchFields) => {
    ...
    const requireExpectedSelected = selectedBatchFields.find(
        (bf) => bf.apiName === DI_BATCH_REQUIRED_TOTAL_TO_MATCH_INFO.fieldApiName
    );
    ...
    requiredFields.forEach((field) => {
        if (selectedFieldsExists) {
            const alreadySelected = selectedBatchFields.find((bf) => {
                return bf.apiName === field.apiName;
            });
            if (!alreadySelected) {
...
const buildErrorMessage = (error) => {
    ...
    if (typeof error === "string" || error instanceof String) {
        ...
        } else if (error.detail.output && Array.isArray(error.detail.output.errors)) {
            ...
            if (element.elementType === "field") {
                for (const fieldMappingDevName of element.dataImportFieldMappingDevNames) {
                    ...
                    if (objectName === DATA_IMPORT_INFO.objectApiName) {
                        ...
                        if (
                            (element.fieldApiName === DI_CONTACT1_IMPORTED_INFO.fieldApiName &&
...
const getPageAccess = async () => {
    ...
    return dataImportSettings && dataImportSettings[FIELD_MAPPING_METHOD_FIELD_INFO.fieldApiName] === ADVANCED_MAPPING;
};
...
const isTrueFalsePicklist = (fieldMapping) => {
    if (fieldMapping) {
        return (
            fieldMapping.Target_Field_Data_Type === BOOLEAN_MAPPING &&

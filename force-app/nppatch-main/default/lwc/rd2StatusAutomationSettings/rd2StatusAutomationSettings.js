...
export default class Rd2StatusAutomationSettings extends LightningElement {
    ...
    assignSettings(settings) {
        this.numberOfDaysForClosed = settings.numberOfDaysForClosed
            ? settings.numberOfDaysForClosed
            : this.labels.commonNoneSpecified;
        this.inputDaysForClosed = settings.numberOfDaysForClosed;

        this.numberOfDaysForLapsed = settings.numberOfDaysForLapsed
            ? settings.numberOfDaysForLapsed
            : this.labels.commonNoneSpecified;
        this.inputDaysForLapsed = settings.numberOfDaysForLapsed;

        this.assignInputDaysThreshold();
        this.closedStatusOption = settings.closedStatusOption;
        this.lapsedStatusOption = settings.lapsedStatusOption;
        this.closedStatus = settings.closedStatus;
        this.lapsedStatus = settings.lapsedStatus;

        this.assignInputStatusWithApiValue(settings);
    ...
    assignInputStatusWithApiValue(settings) {
        this.inputClosedStatus = settings.closedStatusOption.filter(
            (option) => option.label === this.closedStatus
        )[0]?.value;
        this.inputLapsedStatus = settings.lapsedStatusOption.filter(
            (option) => option.label === this.lapsedStatus
        )[0]?.value;
    }

...
const createGeGiftEntryFormApp = () => {
    return createElement("c-ge-gift-entry-form-app", { is: GeGiftEntryFormApp });
};
...
describe("c-ge-gift-entry-form-app", () => {
    ...
    describe("save, update, and delete behavior", () => {
        it("should call addGiftTo with expected arguments when a gift is saved in batch mode", async () => {
            ...
            const formApp = setupForBatchMode({
                giftBatchId: "DUMMY_BATCH_ID",
                gifts: [{ fields: { [FAILURE_INFORMATION.fieldApiName]: null } }],
                totals: { TOTAL: 1 },
            });
            await flushPromises();
        ...
        it("should call expected methods when a gift save fails in batch mode", async () => {
            addGiftTo.mockResolvedValue({});
            const formApp = setupForBatchMode({
                gifts: [{ fields: { [FAILURE_INFORMATION.fieldApiName]: "dummy fail reason" } }],
                totals: { TOTAL: 1 },
            });
            await flushPromises();
    ...
    describe("field change behavior", () => {
        ...
        it("when opportunity record type changes, sets picklist values", async () => {
            ...
            getObjectInfo.emit(opportunityObjectDescribeInfo, (config) => {
                return (
                    config.objectApiName?.objectApiName === OPPORTUNITY_OBJECT.objectApiName ||
            ...
            expect(opportunityTypeInput.options).toContainOptions([
                "c.stgLabelNone",
                "Donation Test",
                "Major Gift Test",
            ]);
        ...
        it("when selected donation has mismatched donor data", async () => {
            ...
            const rendererButton = shadowQuerySelector(
                geFormRenderer,
                'lightning-button[data-qa-locator="button c.geButtonSaveNewGift"]'
            );
    ...
    describe("donor fields validation validation", () => {
        it("when donor type is contact and contact fields not filled in, on save displays error", async () => {
            ...
            expect(pElement.textContent).toBe(
                "When you select Contact1 for Donor Type, you must enter information in Existing Donor Contact"
            );
        });
